# src/infrastructure/model/landmarks_tensorrt_model_adapter.py
"""
Adapter para modelos TensorRT de detecção de landmarks faciais.
Implementa a interface ILandmarksModel usando TensorRT.
"""
...
class LandmarksTensorRTModelAdapter(ILandmarksModel):
    """
    Adapter que encapsula um modelo TensorRT para detecção de landmarks faciais.
    Processa crops de faces e retorna landmarks (keypoints).
    """
    
    def __init__(self, model_path: str, device: str = 'cuda:0', image_size: int = 640):
        """
        Inicializa o adapter com um modelo TensorRT.
        
        :param model_path: Caminho para o arquivo do modelo TensorRT (.engine).
        :param device: Dispositivo padrão para inferência (cuda:0, cuda:1, etc).
        :param image_size: Tamanho de imagem para inferência (640 ou 1280).
        """
        self.model_path = model_path
        self.device = device if device != 'cpu' else 'cuda:0'  # TensorRT requer CUDA
        self.image_size = image_size
        
        try:
            # Carrega modelo TensorRT
            self.model = YOLO(model_path)
            logger.info(f"Modelo TensorRT de landmarks carregado: {model_path}")
        except Exception as e:
            logger.error(f"Erro ao carregar modelo TensorRT: {e}")
            raise
    ...
    def _detect_num_keypoints(self) -> int:
        ...
        try:
            results = self.model(dummy_image, verbose=False, device=self.device)
    ...
    def predict(
        ...
        :param device: Device para inferência (cuda:0, cuda:1, etc). Se None, usa device padrão.
        ...
        try:
            ...
            if verbose:
                logger.error(f"Erro na inferência de landmarks TensorRT: {e}")
            return None
    ...
    def predict_batch(
        ...
        """
        Detecta landmarks em múltiplos crops de face usando TensorRT (batch).
        ...
        try:
            ...
            # TensorRT falha quando imagens têm dimensões diferentes no batch
            normalized_crops = self._normalize_batch_sizes(face_crops)
            
            # TensorRT aceita lista de imagens para batch inference com imgsz configurado
            results = self.model(normalized_crops, conf=conf, verbose=verbose, device=inference_device, imgsz=self.image_size)
            ...
            if verbose:
                logger.error(f"Erro na inferência em batch TensorRT: {e}")
            return [None] * len(face_crops)

    def get_model_info(self) -> dict:
        """
        Retorna informações sobre o modelo TensorRT de landmarks.
        ...
        return {
            ...
            'precision': 'TensorRT Optimized',
            'format': 'TensorRT Engine',
            'optimization': 'NVIDIA GPU Optimized'
        }
